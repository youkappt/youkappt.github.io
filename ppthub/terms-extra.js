...
const TERM_EXTRA = {
  ...
  'coauthor': {
    ...
    shortcut: '存到 OneDrive/SharePoint → 共享 → 共同创作',
    mnemonic: '共同创作＝「同一份，多人并行」。'
  },
  'text-margin': {
    pitfall: '用回车空格硬挤内边距，换框就乱。',
    shortcut: '右键文本框 → 设置形状格式 → 文本框 → 内部边距',
  ...
  'bg-format': {
    ...
    shortcut: '右键页面 → 设置背景格式（可「应用到全部」）',
    mnemonic: '背景格式＝「铺底不挡内容」。'
  },
  'action-settings': {
    pitfall: '只知超链接，不会悬停触发和加音效，交互做不活。',
    shortcut: '选中对象 → 插入 → 动作（或右键「超链接」旁）',
  ...
  'color-603010': {
    pitfall: '点缀色用超 10%，画面花、重点被淹。',
    combo: '60-30-10 + 色彩心理：先定情绪再按比例铺。',
    beforeafter: '改前：五色均等没主次 → 改后：主60辅30点10，稳而跳。',
    mnemonic: '60-30-10＝「大面积定调、小面积点睛」。'
  },
  'autoshape': {
  ...
  '3d-model': {
    pitfall: '用平面图展示产品/结构，没立体感显低端。',
    shortcut: '插入 → 3D 模型（.glb/.fbx，365 支持）',
    combo: '3D 模型 + 转盘动画：慢慢自转更高级。',
    beforeafter: '改前：死的平面图 → 改后：可拖动旋转的 3D。',
    mnemonic: '3D 模型＝「能转的立体图形」。'
  },
  'zoom': {
    pitfall: '只能线性顺序放映，不能按听众兴趣跳。',
    shortcut: '插入 → 缩放定位（摘要/幻灯片/节缩放）',
    combo: '摘要缩放 + 演讲者视图：现场自由导航。',
    beforeafter: '改前：封面→1→2→3 死顺序 → 改后：点章节缩略图跳。',
    mnemonic: '缩放定位＝「用缩略图当目录跳转」。'
  },
  'watermark': {
  ...
  'bullets': {
    pitfall: '要点写成一大段，扫读费劲。',
    shortcut: '开始 → 项目符号/编号（可换图标）',
    combo: '项目符号 + 段落间距：条理清、好读。',
    beforeafter: '改前：一整段文字 → 改后：圆点列表一屏扫完。',
    mnemonic: '项目符号＝「每行前面的小标记」。'
  },
  'pptx-format': {
  ...
  'narrate': {
    pitfall: '想发带讲解的片子只能另开录屏对嘴型，桌面杂音也录进去。',
    shortcut: '幻灯片放映 → 录制幻灯片放映 / 排练计时',
    combo: '录制旁白 + 排练计时：讲一遍存好声和节奏，自动播。',
    beforeafter: '改前：外录屏对嘴型 → 改后：内录旁白自播放。',
    mnemonic: '录制旁白＝「把讲解焊进 PPT」。'
  },
  'outline': {
    pitfall: '直接在画布上逐个文本框敲，结构乱、改标题累、调顺序更烦。',
    shortcut: '视图 → 大纲视图（或左侧缩略图旁切到「大纲」）',
    combo: '大纲视图 + SmartArt：先列层级再一键转图示。',
    beforeafter: '改前：画布上零散文本框 → 改后：左侧层级清晰、拖拽即排序。',
    mnemonic: '大纲视图＝「先写结构，再美排版」。'
  },
  'word-to-ppt': {
  ...
  'crop-shape': {
    pitfall: '想让图片变圆形头像只会用形状遮，边还是方的。',
    shortcut: '图片格式 → 裁剪 → 裁剪为形状 → 选预设',
    combo: '裁剪为形状 + 删除背景：异形窗套图，去背再精修。',
    beforeafter: '改前：方图硬放 → 改后：圆形/六边形异形窗。',
    mnemonic: '裁剪为形状＝「给图片套个异形窗」。'
  },
  'ink': {
  ...
  'qat': {
    pitfall: '高频命令藏在功能区深处，每次翻菜单找半天。',
    shortcut: '右键任意命令 → 添加到快速访问工具栏',
    combo: '快速访问栏 + 快捷键：顶栏钉按钮，键盘流互补。',
    beforeafter: '改前：翻功能区找命令 → 改后：顶栏一键点。',
    mnemonic: '快速访问栏＝「把常用命令钉到顶栏」。'
  }
};
